...
QList<QPoint> Game::BFS(QPoint start, QPoint end, bool blocked) {
  ...
  while (!to_visit.empty()) {
    ...
    for (auto i : all_neighbors) {
      if (i.x() < total_cols && i.x() >= 0 && i.y() < total_rows &&
          i.y() >= 0 && isPath(i.x(), i.y()) &&
...
void Game::createGameControls() {
  ...
  //                            QSizeF(upgradeButton->boundingRect().width() +
  //                            20, upgradeButton->boundingRect().height())));

  int uxPos = this->width() - upgradeButton->boundingRect().width() -
              menuButton->boundingRect().width() - 80;
  ...
  build_CSstudent->setIcon(
      QIcon(QPixmap(":/images/CStudent1.png")
                .scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
  ...
  // create the icon for the button
  build_TA->setIcon(QIcon(
      QPixmap(":/images/TA.png").scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
  ...
  // create the icon for the button
  build_SE->setIcon(QIcon(QPixmap(":/images/Bing.png")
                              .scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
  ...
  // create the icon for the button
  build_LS->setIcon(QIcon(QPixmap(":/images/Language_server.png")
                              .scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
  ...
  // create the icon for the button
  build_Valgrind->setIcon(
      QIcon(QPixmap(":/images/Valgrind.png")
                .scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
  ...
  // create the icon for the button
  build_Comment->setIcon(
      QIcon(QPixmap(":/images/Comment.png")
                .scaled(BUILD_BUTTON_SIZE, BUILD_BUTTON_SIZE)));
...
// creates a wave of enemies for one level according to the description in
// wave.txt
void Game::createWave() {
  ...
  for (QString typeOfEnemy : waves_[level_]) {
    ...
    if (stream.status() == QTextStream::Ok && delay >= 0) {
      ...
      // timer to initiate the next enemy type after the current ones have
      // spawned
      QTimer* nextEnemiesTimer = new QTimer(this);
      nextEnemiesTimer->setSingleShot(true);
      nextEnemiesTimer->setInterval((amount + 1) * delay);
      nextEnemiesTimer->callOnTimeout(
          [this, amount]() { this->addSpawnedEnemies(amount); });
      ...
      if (flag) {
        ...
        connect(timerBuffer, SIGNAL(timeout()), nextEnemiesTimer,
                SLOT(start()));
      }
...
void Game::spawnEnemy(int type) {
  ...
  if (type == 1 || type == 2) {
    CompilerError* enemy = new CompilerError(
        static_cast<CompilerErrorType>(type), path, shortest_path_);
    addEnemy((Enemy*)enemy);

  } else if (type == 3 || type == 4 || type == 5 || type == 6) {
    MemoryError* enemy = new MemoryError(static_cast<MemoryErrorType>(type),
                                         path, shortest_path_);
    addEnemy((Enemy*)enemy);

  } else if (type == 7) {
    RuntimeError* enemy = new RuntimeError(static_cast<RuntimeErrorType>(type),
                                           path, shortest_path_);
...
void Game::updateLeaderboard() {
  ...
  if (stream.atEnd()) {
    bool ok;
    QString text =
        QInputDialog::getText(nullptr, tr("QInputDialog::getText()"),
                              tr("You got a TOP10 score, enter name:"),
    ...
    if (fullLeaderboard.length() >= 10) {
      auto min =
          std::min_element(fullLeaderboard.begin(), fullLeaderboard.end(),
                           [](QPair<QString, int> a, QPair<QString, int> b) {
      ...
      if (score_ <= min->second) {
        ...
        QString text = QInputDialog::getText(
            nullptr, tr("Yay!"), tr("You got a TOP10 score, enter name:"),
            QLineEdit::Normal, QDir::home().dirName(), &ok);
        ...
        void Game::spawnEnemy(int type, QList<QPointF> path) {
          if (type == 1 || type == 2) {
            ...
            MemoryError* enemy = new MemoryError(
                static_cast<MemoryErrorType>(type), path, shortest_path_);
            addEnemy((Enemy*)enemy);
          }
          std::sort(fullLeaderboard.begin(), fullLeaderboard.end(),
                    [](QPair<QString, int> a, QPair<QString, int> b) {
                      return a.second > b.second;
                    });
          file.seek(0);
      ...
      // converting grid matrix coordinates to scene coordinates for the enemie
      // path
      ...
      bool Game::isLost() const { return health_ <= 0; }
      ...
      int Game::getHealth() const { return health_; }

      int Game::getCurrency() const { return currency_; }

      int Game::getEnemyCount() const { return enemyCount_; }

      int Game::getLevel() const { return level_; }

      int Game::getScore() const { return score_; }

      Modes::MODES Game::getMode() const { return mode_; }

      TowerTypes::TYPES Game::getBuildType() const { return buildType_; }
      ...
      void Game::setMode(Modes::MODES m) { mode_ = m; }

      void Game::advanceLevel() { level_++; }
      ...
      void Game::addEnemy(Enemy * enemy) {
        ...
        connect(enemy, SIGNAL(addedEnemy(Enemy*)), this,
                SLOT(addEnemy(Enemy*)));
      ...
      void Game::showMenu() { view->showMenu(); }
      ...
      bool Game::buildTower(int row, int column) {
        ...
        if (dynamic_cast<Tower*>(widget)) {
          ...
          QGraphicsWidget* tower =
              this->addWidget(new Tower(row, column, nullptr));
        ...
        if (dynamic_cast<Tower*>(widget) || dynamic_cast<Comment*>(widget)) {
          ...
          switch (type) {
            case TowerTypes::CS_Student: {
              // create a new tower and add it to the scene
              QGraphicsWidget* tower =
                  this->addWidget(new CS_Student(row, column, nullptr));
            ...
            case TowerTypes::TA: {
              // create a new tower and add it to the scene
              QGraphicsWidget* tower =
                  this->addWidget(new TA(row, column, nullptr));
            ...
            case TowerTypes::Comment: {
              QGraphicsWidget* tower =
                  this->addWidget(new Comment(row, column, 5, nullptr));
      ...
      void Game::enterBuildCom() {
        ...
        build_Comment->setStyleSheet(
            "background-color: rgb(0,255,0); border: 1px solid black");
      }

      void Game::updatePaths() {
        foreach (Enemy* enemy, activeEnemies_) {
          QList<QPoint> newMatrixPath =
              getShortestPath(enemy->getMatrixLocation());
          enemy->setPath(newMatrixPath, convertCoordinates(newMatrixPath));
        }
      }

      void Game::updateEnemyCount() { enemyCount_ = activeEnemies_.length(); }
      ...
      void Game::addSpawnedEnemies(int amount) { spawnedThisWave_ += amount; }
